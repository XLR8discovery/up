...
var frontendDict = map[string]mount{
	"web.satellite": {
		...
		targetPath: "/var/lib/oss/oss/web/satellite",
	},
	"web.multinode": {
		service:    []string{"storagenode"},
		targetPath: "/var/lib/oss/web/multinode",
	},
	"web.storagenode": {
		service:    []string{"storagenode"},
		targetPath: "/var/lib/oss/web/storagenode",
	},
	"admin.ui": {
		service:    []string{"satellite-admin"},
		targetPath: "/var/lib/oss/oss/satellite/admin/ui",
	},
}
...
func localBinCmd() *cobra.Command {
	mountCmd := &cobra.Command{
		...
		RunE: cmd.ExecuteOSSUP(func(st recipe.Stack, rt runtime.Runtime, selector []string) error {
...
func mountBinaries(composeService *types.ServiceConfig) error {
	...
	target := filepath.ToSlash(filepath.Join("/var/lib/oss/go/bin", execName))
